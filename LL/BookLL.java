...
//   Validates an categoriess object and prints errors to console
  ...
  public int getAuthorBooksCount(int id){
    Node temp=head;
    int count=0;
    while(temp!=null){
      if(temp.object.getAuthor_id()==id){
        count++;
      }
      temp=temp.next;
    }
  ...
  public void deleteBook(int Book_id){
    if (head.object.getBook_id()==Book_id){
      Node temp=head;
      head=temp.next;
      return;
    }
    ...
    while(temp.next!=null){
      if(temp.next.object.getBook_id()==Book_id){
  ...
  public boolean findById(int id, int cat_id){
      if(size==0){
          return false ;
      }
      if (id<=0){
      ...
      Node temp=head;
      while(temp!=null){
          if(temp.object.getBook_id()==id && temp.object.getCategory_id()==cat_id) return true;
          temp=temp.next;
      }
      return false;
  }
  ...
  public void Setid(){
    ...
    Node temp=head;
    while(temp!=null){
        if(temp.object.getBook_id()>max_id) max_id=temp.object.getBook_id();
        temp=temp.next;
    }
    Book.setId(max_id+1);
  }
