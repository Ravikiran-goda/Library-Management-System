...
public class LoanLL implements Serializable{
    ...
    public boolean display(int id){
        ...
        boolean check=false;
        while(temp!=null){
            if(temp.object.getStudent_id()==id && temp.object.getReturnDate()==null){
    ...
    public int getLoanBooksCount(int Book_id){
        int count =0;
        Node temp=head;
        while(temp!=null){
            if(temp.object.getBookId()==Book_id && temp.object.getReturnDate()==null){
                count++;
            }
            temp=temp.next;
        }
    ...
    public void SetReturnDate(int BookId, int loan_id){
        Node tem=head;
        while(tem!=null){
            if(tem.object.getLoanId()==loan_id){
    ...
    public void Setid(){
        ...
        Node temp=head;
        while(temp!=null){
            if(temp.object.getLoanId()>max_id) max_id=temp.object.getLoanId();
            System.out.println(temp.object);
            temp=temp.next;
        }
        Loan.setNextId(max_id+1);
      }
